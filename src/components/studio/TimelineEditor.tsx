...
export const TimelineEditor: React.FC<TimelineEditorProps> = ({
  ...
  return (
    <div className="flex-1 w-full bg-[#F8F9FA] text-slate-800 flex flex-col font-sans select-none min-h-[calc(100vh-56px)] animate-fadeIn">
      {/* ========================================================================================= */}
      {/* TOP HEADER BAR (精确还原截图顶栏布局与微件配置)                                            */}
      {/* ========================================================================================= */}
      <div className="bg-white border-b border-slate-200 px-5 py-2.5 flex items-center justify-between shadow-xs sticky top-0 z-30 flex-wrap gap-2">
        {/* Left: Back, Episode Title & Hook Point Target */}
        <div className="flex items-center space-x-3 flex-wrap">
          ...
          <div className="flex items-center space-x-2 font-bold text-sm text-slate-800">
            <span className="bg-slate-900 text-white text-[11px] px-2 py-0.5 rounded-md font-mono">
              EP{episode.episode_number}
            </span>
            <span className="font-extrabold">{episode.title || `第 ${episode.episode_number} 集`}</span>
          </div>

          {episode.hook_point && (
            <div className="hidden lg:flex items-center gap-1 bg-orange-50 border border-orange-200 text-orange-800 text-[11px] font-bold px-2.5 py-0.5 rounded-full">
              <Bookmark className="w-3 h-3 text-orange-600" />
              <span>本集黄金卡点目标：{episode.hook_point}</span>
            </div>
          )}

          {/* Project Visual DNA Badge */}
          <div className="hidden xl:flex items-center gap-1 bg-purple-50 border border-purple-200 text-purple-700 text-[11px] font-bold px-2.5 py-0.5 rounded-md">
            <span className="w-1.5 h-1.5 rounded-full bg-purple-600" />
            <span>画风基因: {project.style_preset || "3D 国漫大作"}</span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2.5 text-xs">
          ...
          <button
            ...
            className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold px-3.5 py-1.5 rounded-lg cursor-pointer transition-all shadow-sm shadow-orange-500/20 flex items-center space-x-1.5 disabled:opacity-50 text-[11px]"
          >
            {isRenderingAll ? (
              <>
      ...
      {/* ========================================================================================= */}
      <div className="flex-1 grid grid-cols-12 gap-0 bg-[#F8FAFC] overflow-hidden min-h-[490px]">
        {/* --------------------------------------------------------------------------------------- */}
        ...
        <div className="col-span-12 md:col-span-5 lg:col-span-5 p-5 flex flex-col justify-between space-y-4">
          ...
          <div className="flex items-center justify-end space-x-2 pt-2 border-t border-slate-200">
            ...
            <button
              ...
              className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold px-6 py-2 rounded-xl text-xs transition-all shadow-md shadow-orange-500/20 cursor-pointer disabled:opacity-50 flex items-center space-x-1.5"
            >
              {isRenderingShot ? (
                <>
